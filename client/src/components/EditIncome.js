...
const EditIncome = () => {
    ...
    const {displayAlert,editTransaction}= useGlobalContext();

    const initialValues= {
        title:editTransaction? editTransaction[0].title:"",
        information:editTransaction? editTransaction[0].information:"",
        amount:editTransaction?editTransaction[0].amount:"",
        category:editTransaction?editTransaction[0].category:"",
        status:editTransaction?editTransaction[0].status:""
    }
...
const handleChange=(e)=>{
  ...
  return (
    <div className='w-9/12 mx-auto bg-white mt-8 rounded p-6'>
        <h1 className='bg-indigo-300 text-white text-center my-2 py-2 rounded'>Edit Income</h1>
